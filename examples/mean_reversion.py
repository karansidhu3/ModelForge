"""End-to-end example: load a CSV, run the mean-reversion factor through the
pipeline, inspect the validated result (including the moving-average
intermediate).
...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        factor = MeanReversionFactor(window=WINDOW)
        params = MeanReversionParams(window=WINDOW)
    ...
    print()
    print("Moving average (intermediate):")
    print(result.factor_result.intermediates["moving_average"].to_string())
    print()
    print("Deviation from moving average:")
