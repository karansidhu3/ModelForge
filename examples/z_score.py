"""End-to-end example: load a CSV, run the z-score factor through the
pipeline, inspect the validated result (including the rolling mean/std
intermediates).
...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        factor = ZScoreFactor(window=WINDOW)
        params = ZScoreParams(window=WINDOW)
    ...
    print()
    print("Rolling mean (intermediate):")
    print(result.factor_result.intermediates["rolling_mean"].to_string())
    print()
    print("Rolling std (intermediate):")
    print(result.factor_result.intermediates["rolling_std"].to_string())
    print()
    print("Z-score:")
